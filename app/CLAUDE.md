...
- `page.tsx` — Home route (`/`). A Server Component that does nothing but `redirect('/canvas')` from `next/navigation`, which in turn forwards to the most recent workspace (or an unsaved playground canvas when there is none).
